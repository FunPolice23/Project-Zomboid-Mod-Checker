...
# ── Static B42 fix database ───────────────────────────────────────────────────
# Each entry: pattern (substring match on error message) → fix advice
QUICK_FIX_DB: list[dict] = [
    # ── Lua fragile calls ──────────────────────────────────────────────────
    {
        "match":    "getPlayer",
        "category": "Lua – Fragile Call",
        "problem":  "getPlayer() assumes singleplayer and returns nil in multiplayer.",
        "fix":      "Replace with getSpecificPlayer(0) for singleplayer, or pass the player object through event arguments.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidLuaDocs/namespaceumbrella.html",
    },
    {
        "match":    "SandboxVars",
        "category": "Lua – Fragile Call",
        "problem":  "SandboxVars access pattern changed in B42.",
        "fix":      "Use SandboxVars.<OptionName> directly. Ensure your sandbox option is defined in sandbox.txt with correct syntax.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidLuaDocs/md_Events.html",
    },
    {
        "match":    "getCore",
        "category": "Lua – Fragile Call",
        "problem":  "getCore().getOption() and similar calls may be removed or renamed in B42.",
        "fix":      "Check the B42 JavaDocs for the current Core API. Many options moved to SandboxVars or GameSettings.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidJavaDocs/",
    },
    {
        "match":    "ISBuildingMenu",
        "category": "Lua – Fragile Call",
        "problem":  "ISBuildingMenu was restructured in B42.",
        "fix":      "Use the new BuildingMenu API. Register recipes via BuildingMenu.addRecipe() and check B42 modding examples.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidLuaDocs/annotated.html",
    },
    {
        "match":    "ISTimedActionQueue",
        "category": "Lua – Fragile Call",
        "problem":  "ISTimedActionQueue API changed in B42.",
        "fix":      "Pass the specific player object: ISTimedActionQueue.add(player, action). Do not use global player references.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidLuaDocs/annotated.html",
    },
    {
        "match":    "keyBinding",
        "category": "Lua – Key Binding",
        "problem":  "Key binding registration changed significantly in B42.",
        "fix":      "Use the new InputSystem. Register keys via InputSystem.addKeyBinding() instead of the old Keyboard table.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidLuaDocs/md_Callbacks.html",
    },
    {
        "match":    "ISContextMenu",
        "category": "Lua – Context Menu",
        "problem":  "ISContextMenu and ISInventoryContextMenu were refactored in B42.",
        "fix":      "Use ISWorldObjectContextMenu.addOption() / ISInventoryPage context hooks via the new event signatures.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidLuaDocs/md_Events.html",
    },
    # ── Deprecated hooks ──────────────────────────────────────────────────
    {
        ...
        "fix":      "Use OnFillWorldObjectContextMenu with BuildingMenu checks, or the new recipe system hooks.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidLuaDocs/md_Events.html",
    },
    {
        "match":    "OnFillInventoryObjectContextMenu",
        "category": "Lua – Changed Event",
        "problem":  "Signature changed in B42 — player index argument added.",
        "fix":      "Update function signature to: function(playerIndex, context, items). playerIndex is 0 in singleplayer.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidLuaDocs/md_Events.html",
    },
    {
        "match":    "removed or changed in Build 42",
        "category": "Lua – Unknown Event",
        "problem":  "This event does not exist in the B42 event list.",
        "fix":      "Check the full B42 events list. The event may have been renamed, merged, or removed. Search the B42 Lua docs for the nearest equivalent.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidLuaDocs/md_Events.html",
    },
    # ── Java – missing class ───────────────────────────────────────────────
    {
        ...
        "problem":  "The mod references a game class that no longer exists in B42.",
        "fix":      "Search the B42 JavaDocs for a replacement class. The class may have been moved to a different package or merged with another class.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidJavaDocs/",
    },
    # ── Java – missing method ─────────────────────────────────────────────
    {
        ...
        "problem":  "The method was removed or renamed in B42.",
        "fix":      "Check the B42 JavaDocs for the class. Look for a method with similar purpose. The signature (parameter types) may have changed.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidJavaDocs/",
    },
    {
        "match":    "Method signature changed",
        "category": "Java – Signature Changed",
        "problem":  "The method exists but its parameter types or return type changed in B42.",
        "fix":      "Update the call to match the new signature shown in the error. Common change: extra player/context argument added.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidJavaDocs/",
    },
    # ── Java – missing field ──────────────────────────────────────────────
    {
        ...
        "problem":  "A field (class variable) was removed or renamed in B42.",
        "fix":      "Check the B42 JavaDocs for the class. The field may have been made into a method (getter/setter) or moved to a parent class.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidJavaDocs/",
    },
    # ── Java – visibility ─────────────────────────────────────────────────
    {
        ...
        "problem":  "A method or field changed from public to protected/private in B42.",
        "fix":      "You may need to use reflection, or find an officially exposed API method that wraps the same logic.",
        "docs":     "https://demiurgequantified.github.io/ProjectZomboidJavaDocs/",
    },
]
...
def _find_fixes(message: str) -> list[dict]:
    """Return all matching fix entries for a given error message."""
...
class QuickFixTab:
    """
    Shows fix suggestions for the current scan results.
    Call QuickFixTab.refresh(issues) after each scan to update.
    """

    # Stored so gui.py can call it after a scan
    _refresh_fn = None
    ...
    def build(parent_widget: QWidget):
        ...
        hint = QLabel("Run a compatibility check first, or load a saved report — suggestions appear automatically.")
        hint.setWordWrap(True)
        layout.addWidget(hint)

        load_row = QHBoxLayout()
        load_report_btn = QPushButton("📂 Load from Saved Report (.txt)")
        load_row.addWidget(load_report_btn)
        load_row.addStretch()
        ...
        filter_row = QHBoxLayout()
        filter_box = QLineEdit()
        filter_box.setPlaceholderText("🔍 Filter by keyword…")
        cat_combo = QComboBox()
        cat_combo.addItem("All Categories")
        for cat in sorted({f["category"] for f in QUICK_FIX_DB}):
            cat_combo.addItem(cat)
        filter_row.addWidget(filter_box, stretch=1)
        ...
        issue_tree.setHeaderLabels(["Severity", "Category", "Source"])
        ...
        detail_box.setFont(QFont("Consolas", 11))
        detail_box.setPlaceholderText("Select an issue to see fix suggestions…")
        ...
        def _populate_tree(issues: list[dict]):
            ...
            for issue in issues:
                ...
                category = fixes[0]["category"] if fixes else "Unknown"
                if cat_filter != "All Categories" and category != cat_filter:
                    continue

                item = QTreeWidgetItem([sev, category, src])
                item.setData(0, Qt.ItemDataRole.UserRole, (msg, fixes))

                if sev == "ERROR":
                    item.setBackground(0, QColor(100, 0, 0))
                else:
                    item.setBackground(0, QColor(120, 60, 0))
            ...
            status.setText(f"Showing {shown} of {len(issues)} issue(s).")

        def _on_select(item: QTreeWidgetItem, _col: int):
            msg, fixes = item.data(0, Qt.ItemDataRole.UserRole)
            lines = [f"Issue:\n  {msg}\n"]
            if fixes:
                for fix in fixes:
                    ...
                    lines.append(f"Docs:      {fix['docs']}")
                    lines.append("")
            else:
                lines.append("No specific fix suggestion for this issue.")
                lines.append("Check the B42 JavaDocs or Lua event list for guidance.")
                lines.append("https://demiurgequantified.github.io/ProjectZomboidJavaDocs/")
        ...
        def _parse_report_file(path: str) -> list[dict]:
            ...
            try:
                ...
                for m in _re2.finditer(r'\[(ERROR|WARNING)\]\s+(.+?)\n\s+at\s+(.+?)(?=\n|$)', text, _re2.S):
                    sev, msg, src = m.group(1), m.group(2).strip(), m.group(3).strip()
                    issues.append({"severity": sev, "message": msg, "source": src})
            except Exception as e:
                QMessageBox.warning(parent_widget, "Load Report", f"Could not parse report:\n{e}")
            return issues
        ...
        def _load_report():
            ...
            if path:
                ...
                if issues:
                    ...
                    status.setText("No issues found in report — check file format.")

        load_report_btn.clicked.connect(_load_report)
    ...
    def refresh(issues: list[dict]):
        """Call this from gui.py after each scan completes."""
